...
class GenerateItemsTest(unittest.TestCase):
    ...
    def test_elevenlabs_usage_and_row(self) -> None:
        ...
        self.assertEqual(self.el_calls, [("hi", "vc-42", "eleven_turbo_v2_5", "en", str(server.TMP / "a3.mp3"))])
    ...
    def test_output_files_and_manifest_row(self) -> None:
        ...
        row = rows[0]
        self.assertEqual(row["wav"], "a8.wav")
        self.assertEqual(row["ogg"], "a8.ogg")
        self.assertEqual(row["duration_sec"], 1.0)
        self.assertTrue((server.READY / "a8.wav").exists())
